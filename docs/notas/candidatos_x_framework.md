# Candidatos bibliométricos por Framework SCAR

Generado por `scripts/enrich_candidatos_framework.py`.

## (sin asignar) — 1 candidatos

| Sitio | Pubs | Menciones | Fuente asignación |
|---|---:|---:|---|
| Pacific-Antarctic Ridge | 5 | 6 | fuera de bboxes conocidos |

## F1 Basement — 12 candidatos

| Sitio | Pubs | Menciones | Fuente asignación |
|---|---:|---:|---|
| Prydz Bay | 30 | 257 | soft-rule (regla geográfica) |
| Larsemann Hills | 18 | 142 | soft-rule (regla geográfica) |
| Prince Charles Mountains | 15 | 31 | soft-rule (regla geográfica) |
| Bunger Hills | 15 | 64 | soft-rule (regla geográfica) |
| Ellsworth Mountains | 14 | 75 | soft-rule (regla geográfica) |
| Lambert Glacier | 11 | 33 | soft-rule (regla geográfica) |
| Windmill Islands | 11 | 43 | soft-rule (regla geográfica) |
| Lützow-Holm Bay | 9 | 27 | soft-rule (regla geográfica) |
| Taylor Glacier | 9 | 58 | soft-rule (regla geográfica) |
| Denman Glacier | 7 | 31 | soft-rule (regla geográfica) |
| Shackleton Range | 6 | 23 | soft-rule (regla geográfica) |
| Whitmore Mountains | 5 | 10 | soft-rule (regla geográfica) |

## F3 Beacon Supergroup (cobertura Gondwana) — 2 candidatos

| Sitio | Pubs | Menciones | Fuente asignación |
|---|---:|---:|---|
| Transantarctic Mountains | 48 | 200 | soft-rule (regla geográfica) |
| Shackleton Glacier | 5 | 12 | soft-rule (regla geográfica) |

## F2: Proterozoic orogens & rifted margins — 3 candidatos

| Sitio | Pubs | Menciones | Fuente asignación |
|---|---:|---:|---|
| Wright Valley | 9 | 33 | spatial-join (antartica_frameworks.geojson) |
| Frontier Mountain | 5 | 5 | spatial-join (antartica_frameworks.geojson) |
| Bingen | 5 | 27 | spatial-join (antartica_frameworks.geojson) |

## F4 Cenozoic volcanism — 4 candidatos

| Sitio | Pubs | Menciones | Fuente asignación |
|---|---:|---:|---|
| Thwaites Glacier | 12 | 76 | soft-rule (regla geográfica) |
| Pine Island Bay | 9 | 34 | soft-rule (regla geográfica) |
| Thurston Island | 6 | 14 | soft-rule (regla geográfica) |
| Balleny Islands | 5 | 15 | soft-rule (regla geográfica) |

## F4: Active margin & West Antarctic rift — 6 candidatos

| Sitio | Pubs | Menciones | Fuente asignación |
|---|---:|---:|---|
| Seymour Island | 27 | 124 | spatial-join (antartica_frameworks.geojson) |
| Fildes Peninsula | 18 | 185 | spatial-join (antartica_frameworks.geojson) |
| Mount Erebus | 9 | 24 | spatial-join (antartica_frameworks.geojson) |
| Penguin Island | 6 | 16 | spatial-join (antartica_frameworks.geojson) |
| Ross Archipelago | 5 | 6 | spatial-join (antartica_frameworks.geojson) |
| Small Island | 5 | 6 | spatial-join (antartica_frameworks.geojson) |

## F6: Cenozoic glacial history — 1 candidatos

| Sitio | Pubs | Menciones | Fuente asignación |
|---|---:|---:|---|
| Vestfold Hills | 19 | 89 | spatial-join (antartica_frameworks.geojson) |

## F7 Meteorite fields — 1 candidatos

| Sitio | Pubs | Menciones | Fuente asignación |
|---|---:|---:|---|
| Grove Mountains | 17 | 73 | soft-rule (regla geográfica) |

## F4 Arco de la Península Antártica — 20 candidatos

| Sitio | Pubs | Menciones | Fuente asignación |
|---|---:|---:|---|
| South Shetland Islands | 47 | 427 | soft-rule (regla geográfica) |
| King George Island | 43 | 728 | soft-rule (regla geográfica) |
| Deception Island | 27 | 276 | soft-rule (regla geográfica) |
| Livingston Island | 22 | 146 | soft-rule (regla geográfica) |
| James Ross Island | 20 | 140 | soft-rule (regla geográfica) |
| Marguerite Bay | 13 | 56 | soft-rule (regla geográfica) |
| Admiralty Bay | 13 | 187 | soft-rule (regla geográfica) |
| Anvers Island | 11 | 39 | soft-rule (regla geográfica) |
| Alexander Island | 10 | 24 | soft-rule (regla geográfica) |
| Maxwell Bay | 10 | 72 | soft-rule (regla geográfica) |
| Adelaide Island | 9 | 31 | soft-rule (regla geográfica) |
| Greenwich Island | 9 | 31 | soft-rule (regla geográfica) |
| Trinity Peninsula | 9 | 15 | soft-rule (regla geográfica) |
| Vega Island | 8 | 34 | soft-rule (regla geográfica) |
| Fourcade Glacier | 7 | 10 | soft-rule (regla geográfica) |
| Hurd Peninsula | 7 | 20 | soft-rule (regla geográfica) |
| Ulu Peninsula | 6 | 26 | soft-rule (regla geográfica) |
| Chapel Hill | 6 | 13 | soft-rule (regla geográfica) |
| Joinville Island | 6 | 7 | soft-rule (regla geográfica) |
| Hobbs Glacier | 5 | 11 | soft-rule (regla geográfica) |

## F4 Arco de Scotia (sub-antártico) — 2 candidatos

| Sitio | Pubs | Menciones | Fuente asignación |
|---|---:|---:|---|
| Signy Island | 12 | 99 | soft-rule (regla geográfica) |
| South Orkney Islands | 10 | 89 | soft-rule (regla geográfica) |

## TAM Central (Dry Valleys – Royal Society Range) — 4 candidatos

| Sitio | Pubs | Menciones | Fuente asignación |
|---|---:|---:|---|
| Ross Island | 14 | 110 | spatial-join (antartica_frameworks.geojson) |
| Taylor Valley | 14 | 114 | spatial-join (antartica_frameworks.geojson) |
| McMurdo Sound | 11 | 141 | spatial-join (antartica_frameworks.geojson) |
| Mackay Glacier | 6 | 11 | spatial-join (antartica_frameworks.geojson) |

## TAM Norte (Northern Victoria Land) — 2 candidatos

| Sitio | Pubs | Menciones | Fuente asignación |
|---|---:|---:|---|
| Robertson Bay | 10 | 28 | spatial-join (antartica_frameworks.geojson) |
| Inexpressible Island | 6 | 11 | spatial-join (antartica_frameworks.geojson) |

## TAM Pacific (Pensacola-Thiel Mountains) — 1 candidatos

| Sitio | Pubs | Menciones | Fuente asignación |
|---|---:|---:|---|
| Pensacola Mountains | 8 | 14 | spatial-join (antartica_frameworks.geojson) |

## TAM Sur (Beardmore-Shackleton-Nimrod) — 1 candidatos

| Sitio | Pubs | Menciones | Fuente asignación |
|---|---:|---:|---|
| Beardmore Glacier | 7 | 14 | spatial-join (antartica_frameworks.geojson) |

